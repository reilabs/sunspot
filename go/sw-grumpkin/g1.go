...
// then the compiled circuit depends on s. If it is variable type, then
// the circuit is independent of the inputs.
func MultiScalarMul(api frontend.API, Q []*G1Affine, s []interface{}, opts ...algopts.AlgebraOption) *G1Affine {
	if len(Q) != len(s) {
		panic("length of point vector must be same as length of scalar vector")
	}
	output := G1Affine{
		frontend.Variable(0), frontend.Variable(0),
	}
	for i, point := range Q {
		output.AddUnified(api, *point.ScalarMul(api, *point, s[i], opts...))
	}
