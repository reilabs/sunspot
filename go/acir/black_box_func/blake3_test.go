...
func loadBlake3(t *testing.T, path string) BlackBoxFuncCall[*bn254.BN254Field, constraint.U64] {
	t.Helper()
...
func TestBlake3UnmarshalReaderEmpty(t *testing.T) {
	...
	bbf := loadBlake3(t, "../../binaries/black_box_func/blake3/blake3_test_empty.bin")

	expected := BlackBoxFuncCall[T, E]{function: &Blake3[T, E]{
		Inputs:  []FunctionInput[T]{},
		Outputs: [32]shr.Witness{},
	}}
	for i := 0; i < 32; i++ {
		expected.function.(*Blake3[T, E]).Outputs[i] = shr.Witness(0)
	}
...
func TestBlake3UnmarshalReaderWithInputs(t *testing.T) {
	type T = *bn254.BN254Field
	type E = constraint.U64
	bbf := loadBlake3(t, "../../binaries/black_box_func/blake3/blake3_test_with_inputs.bin")
	...
	for i := 0; i < 32; i++ {
		expected.function.(*Blake3[T, E]).Outputs[i] = shr.Witness(1234)
	}
	if !bbf.Equals(&expected) {
		t.Errorf("Expected BlackBoxFuncCall to be %+v, got %+v", expected, bbf)
	}
