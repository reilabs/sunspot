...
func TestBigIntSubUnmarshalReader(t *testing.T) {
	file, err := os.Open("../../binaries/black_box_func/big_int_sub/big_int_sub_test.bin")
	if err != nil {
	...
	expectedFunctionCall := BlackBoxFuncCall[*bn254.BN254Field]{
		Kind: ACIRBlackBoxFuncKindBigIntSub,
		BigIntSub: &BigIntSub{
			Lhs:    1234,
			Rhs:    5678,
			Output: 91011,
