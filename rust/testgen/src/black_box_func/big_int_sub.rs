...
fn generate_big_int_sub_test(path: &str) {
    ...
    let big_int_sub_function_call = BlackBoxFuncCall::<FieldElement>::BigIntSub {
        lhs: 1234,
        rhs: 5678,
        output: 91011,
    };
    ...
    let data = bincode::serde::encode_to_vec(&big_int_sub_function_call, config)
...
pub fn generate_tests(root: &str) {
    ...
    generate_big_int_sub_test(&directory_path);
