...
fn generate_big_int_from_le_bytes_test_empty(path: &str) {
    ...
    let big_int_div_function_call = BlackBoxFuncCall::<FieldElement>::BigIntFromLeBytes {
        inputs: vec![],
        modulus: vec![],
        output: 0,
    };
    ...
    let data = bincode::serde::encode_to_vec(&big_int_div_function_call, config)
...
fn generate_big_int_from_le_bytes_test_with_inputs_and_modulus(path: &str) {
    ...
    // Example data for the 'big_int_from_le_bytes' function call with inputs and modulus
    let big_int_div_function_call = BlackBoxFuncCall::<FieldElement>::BigIntFromLeBytes {
        inputs: vec![FunctionInput::witness(Witness(1), 1234)],
        modulus: vec![1, 2, 3, 4],
        output: 5678,
    };
...
pub fn generate_tests(root: &str) {
    ...
    generate_big_int_from_le_bytes_test_empty(&directory_path);
    generate_big_int_from_le_bytes_test_with_inputs_and_modulus(&directory_path);
