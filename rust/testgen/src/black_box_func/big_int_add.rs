...
fn generate_big_int_add_test(path: &str) {
    let file_name = format!("{}/big_int_add_test.bin", path);
    ...
    let big_int_add_function_call = BlackBoxFuncCall::<FieldElement>::BigIntAdd {
        lhs: 1234,
        rhs: 5678,
        output: 91011,
    };
    ...
    let data = bincode::serde::encode_to_vec(&big_int_add_function_call, config)
