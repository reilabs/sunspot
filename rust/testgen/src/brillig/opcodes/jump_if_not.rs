...
fn generate_jump_if_not_test(path: &str) {
    let file_name = format!("{path}/jump_if_not.bin");
    ...
    let config = bincode::config::standard()
        ...
        .with_little_endian();
    let data =
        bincode::serde::encode_to_vec(&jump_if_not_opcode, config).expect("Failed to encode data");
...
pub fn generate_tests(directory: &str) {
    let directory = format!("{directory}/jump_if_not/");
    // Create the directory if it doesn't exist
    std::fs::create_dir_all(&directory).expect("Failed to create directory");

    // Generate witness tests for binary field operations
    generate_jump_if_not_test(&directory);

    trace!("Jump if not tests generated in {}", directory);
}
