...
use brillig::{BinaryFieldOp, MemoryAddress, Opcode};
use tracing::trace;
...
fn generate_test_binary_field_op_add(path: &str) {
    ...
    let binary_field_op = Opcode::<FieldElement>::BinaryFieldOp {
    ...
    let config = bincode::config::standard()
        ...
        .with_little_endian();
    let data =
        bincode::serde::encode_to_vec(&binary_field_op, config).expect("Failed to encode data");
...
fn generate_test_binary_field_op_less_than_equals(path: &str) {
    ...
    let binary_field_op = Opcode::<FieldElement>::BinaryFieldOp {
        destination: MemoryAddress::Direct(1234),
        op: BinaryFieldOp::LessThanEquals,
        lhs: MemoryAddress::Direct(5678),
        rhs: MemoryAddress::Relative(91011),
    }; // Placeholder for actual data
    let config = bincode::config::standard()
...
pub fn generate_tests(directory: &str) {
    ...
    generate_test_binary_field_op_add(&directory);
    generate_test_binary_field_op_less_than_equals(&directory);

    trace!("Generated binary field operation tests in {}", directory);
}
