...
fn generate_stop_test(path: &str) {
    let file_name = format!("{path}/stop.bin");
    ...
    let stop_opcode = Opcode::<FieldElement>::Stop {
    ...
    let config = bincode::config::standard()
        ...
        .with_little_endian();
    let data = bincode::serde::encode_to_vec(&stop_opcode, config).expect("Failed to encode data");
...
pub fn generate_tests(directory: &str) {
    let directory = format!("{directory}/stop/");
    // Create the directory if it doesn't exist
    std::fs::create_dir_all(&directory).expect("Failed to create directory");

    // Generate witness tests for binary field operations
    generate_stop_test(&directory);

    trace!("Stop tests generated in {}", directory);
}
